...
SELECT hero_id, localized_name, MAX(count) FROM (
    ...
    FROM (
        ...
        FROM (
            SELECT heroes.localized_name, heroes.id as hero_id, mpd.match_id,go.time,row_number() over (partition by mpd.match_id order by go.time)
            as a,row_number() over (partition by  heroes.id, mpd.match_id order by go.time) as b
            FROM heroes
